...
if [ "$MODE" == "dae" ] ; then
    docker run -d -p 5000:5000 --name nerd_room_frontend nerd_room_frontend_img
    ...
    for ((i = 0; i < SERVERS; i++)); do
        ...
        docker run -i -p 127.0.0.1:$OUT_PORT:50051 -v ${VOLUME_NAME}:/code/archive --env ID_SERVER=${i} --env NUM_SERVERS=${SERVERS} --name "${CONTAINER_NAME}" --network nerd_room_net nerd_room_backend_img &
    done
